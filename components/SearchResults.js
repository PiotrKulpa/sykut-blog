...
const SearchResults = ({ posts = [], pageName = "" }) => {
  ...
  const filteredPosts =
    posts && pageName === "search"
      ...
      : posts.filter((el) =>
          el.tags.toLowerCase().includes(searchText.toLowerCase())
        );
  ...
  return (
    <>
      {filteredPosts.length > 0 ? (
        ...
        <p>
          Nie znaleziono wpisów dla hasła: <strong>{searchText}</strong>
        </p>
      )}
      <div className="pagination-layout1 margin-b-30 custom-btn-show-more"></div>
    </>
  );
};
